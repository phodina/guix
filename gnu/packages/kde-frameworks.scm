                        ...
                        (replace 'check
                          (lambda* (#:key tests? #:allow-other-keys)
                            (when tests?
                              ...
                              (invoke "ctest")))))))
    (inputs (list qtbase-5 qtdeclarative-5 qtquickcontrols2-5 xorg-server-for-tests))
...
(define-public breeze-icons
  (package
    ...
    (build-system cmake-build-system)
    (native-inputs (list extra-cmake-modules fdupes
                         `(,gtk+ "bin") python python-lxml)) ;For 24x24 icon generation
    (inputs (list qtbase-5))
    (arguments
     (list #:phases #~(modify-phases %standard-phases
                        (add-after 'install 'update-cache
                          (lambda* _
                            (invoke (string-append (ungexp gtk+ "bin")
...
(define-public karchive
  (package
    ...
    (inputs (list bzip2 qtbase-5 xz zlib
                  `(,zstd "lib")))
    (home-page "https://community.kde.org/Frameworks")
...
(define-public kwayland
  (package
    ...
    (native-inputs
     (list extra-cmake-modules pkg-config))
    (inputs
     (list libxkbcommon qtbase-5 plasma-wayland-protocols qtwayland wayland wayland-protocols))
    (arguments
     `(#:phases
       (modify-phases %standard-phases
...
(define-public kcontacts
  (package
    ...
    (source (origin
              ...
              (patches
               (search-patches
                "kcontacts-Fix-incorrect-name-of-country-in-compariso.patch"))
              (sha256
               (base32
...
(define-public knetworkmanager
  (package
    ...
    (native-inputs (list extra-cmake-modules pkg-config))
    (inputs (list glib qtbase-5))
    (propagated-inputs (list network-manager))
         ...
         (add-after 'unpack 'patch-paths
           (lambda* (#:key inputs outputs #:allow-other-keys)
             ...
             (substitute* "src/kdeinit/kinit.cpp"
               ...
               (("GUIX_PKGS_KF5_PLASMA") #$(this-package-input "plasma-framework"))))))))
    (native-search-paths
